...
def main(config):
    ...
    headlines = get_feed_content(feed_url)

    # If primary feed fails, try others as fallback
    if not headlines:
        headlines = try_other_feeds(feed_name, selected_feeds)
    ...
    return render.Root(
        ...
        child = render.Marquee(
            ...
            child = render.Column(
                expanded = True,
                children = header + [item for sublist in [get_child(h) for h in headlines] for item in sublist],
            ),
        ),
    )
